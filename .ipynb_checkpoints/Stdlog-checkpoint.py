...
class stdlog:
    ...
    def visualize (self, mnemonics,  pltrange, tracks = None, sbsline = None ):
        ...
        matplotlib.rcParams['axes.facecolor'] = '#e6f0e8'
            ...
            for i in range(1,numtracks + 1):
                ...
                # for DEPTH TVD track
                    ...
                    if len(nested_track) == 2:
                        ...
                        host.plot(conn, well['TVD'], alpha = 0)
                        host.set_ylim(mdbounds[0], mdbounds[1])
                        host.set_yticks(np.arange(mdbounds[0], mdbounds[1]+1, 100))
                        host.tick_params(axis ='y', which = 'major', length = 4, pad = -70, direction = 'in', labelsize = 18)
                        host.grid(True, which = 'both', axis = 'y', ls = '-', lw = 1.5)
                        host.invert_yaxis()
...
#                 axdens = fig.axes[denspos]
#                 X1 = well['DENS']
#                 X2 = well['CNL']
#                 y =  well['DEPTH']
#                 axdens.fill_betweenx(y, x1, x2, where = val > x2, interpolate = True, color = '#d6cc0b')
